...
writeFileSync(resolve(output, "heritage-light.json"), `${JSON.stringify(style)}\n`);
writeFileSync(resolve(output, "heritage-light.json.meta"), `${JSON.stringify({ generatedBy: "scripts/generate-map-assets.mjs", release, labelFace: "Noto Sans Regular", omittedPatterns: ["pois", "transit", "airport", "shield"], omittedMelbourneLayers: [...melbourneRedundantLayers], regressionChecks }, null, 2)}\n`);
...
console.log(`Generated ${retained.length} validated heritage-light layers, versioned glyphs/sprites, and the CSP worker.`);
