...
const apply = (state: FreeMapUrlState) => { explorer.query = state.query; explorer.category = state.category; explorer.sort = state.sort; explorer.selectedId = state.point; };
apply(parseUrlState(location.href));
const write = (mode: "replace" | "push") => {
  const url = applyUrlState(new URL(location.href), { query: explorer.query, category: explorer.category, sort: explorer.sort, point: explorer.selectedId });
