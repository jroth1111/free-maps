...
export class FreeMapSurfaceElement extends LitElement {
  ...
  render() { if (this.runtime.phase === "error") return html`<div class="error" part="error" role="alert"><h2>Map unavailable</h2><p>${this.runtime.failure?.message}</p></div>`; return html`<div class="shell" part="shell" aria-busy=${this.runtime.phase === "loading" ? "true" : "false"}><div class="map" part="map"><div class="map-host"></div><div class="map-state" ?hidden=${this.runtime.active}>Map loads when eligible.</div></div></div>`; }
}
